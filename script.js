/* =========================================================
   복복이 로또 웹앱 script.js - 500x500 버전
   ========================================================= */

const LOTTO_MAX = 45;
const PICK_COUNT = 6;

/* 500x500에서는 공이 너무 많으면 답답해서 14개로 조정 */
const IDLE_BALL_COUNT = 14;
...
function setupFakeBalls() {
  ...
  for (let i = 0; i < IDLE_BALL_COUNT; i++) {
    ...
    /* 500x500용 이동 거리. 너무 크게 잡으면 유리통 밖으로 나가 보일 수 있음 */
    ball.style.setProperty("--x1", `${randomBetween(-34, 34)}px`);
    ball.style.setProperty("--y1", `${randomBetween(-20, 20)}px`);
    ball.style.setProperty("--x2", `${randomBetween(-42, 42)}px`);
    ball.style.setProperty("--y2", `${randomBetween(-26, 26)}px`);
    ball.style.setProperty("--x3", `${randomBetween(-38, 38)}px`);
    ball.style.setProperty("--y3", `${randomBetween(-24, 24)}px`);
    ball.style.setProperty("--x4", `${randomBetween(-34, 34)}px`);
    ball.style.setProperty("--y4", `${randomBetween(-22, 22)}px`);
    ball.style.setProperty("--x5", `${randomBetween(-44, 44)}px`);
    ball.style.setProperty("--y5", `${randomBetween(-28, 28)}px`);
...
function announceNumbers(result) {
  ...
  sequence.forEach((item, index) => {
    ...
    }, index * 560); /* 번호 발표 간격. 빠르게 하려면 400, 느리게 하려면 800 */
  });
}
